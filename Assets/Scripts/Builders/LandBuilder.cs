...
public class LandBuilder : MonoBehaviour
{
    ...
    public static void CreateLandTile(int column, int row, byte elevation = 255)
    {
        //creates tile and adds its reference to MapHolder
        byte newElevation = elevation == 255 ? (byte) 0 : elevation;
        if (MapHolder.tiles[column, row] != null)
        {
            if (MapHolder.tiles[column, row].backgroundType == TilePrefabType.Land)
            {
                ...
                MapHolder.tiles[column, row].SetElevation(MapHolder.elevationLevels[newElevation]);
            }
            else
            {
            ...
            if (elevation == 255)
            {
                MapHolder.tiles[column, row].IgnoreElevation(false);
            }
        }
        else
        {
            ...
            MapHolder.tiles[column, row].SetCreatedElevation(MapHolder.elevationLevels[newElevation]);
            MapHolder.tiles[column, row].SetPosition(new Vector3(column, 0, -row));
            MapHolder.tiles[column, row].colliderObject.name = $"{column} {row}";
        }

        MapHolder.tiles[column, row].elevation = newElevation;
